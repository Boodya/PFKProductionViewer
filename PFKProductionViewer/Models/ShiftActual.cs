using System;
using System.Collections.Generic;

#nullable disable

namespace PFKProductionViewer.Models
{
    public partial class ShiftActual
    {
        public int CollPack11 { get; set; }
        public int OveralCollList11 { get; set; }
        public float OveralVList11 { get; set; }
        public int CollPack21 { get; set; }
        public int OveralCollList21 { get; set; }
        public float OveralVList21 { get; set; }
        public int CollPack12 { get; set; }
        public int OveralCollList12 { get; set; }
        public float OveralVList12 { get; set; }
        public int CollPack22 { get; set; }
        public int OveralCollList22 { get; set; }
        public float OveralVList22 { get; set; }
        public int CollPack13 { get; set; }
        public int OveralCollList13 { get; set; }
        public float OveralVList13 { get; set; }
        public int CollPack23 { get; set; }
        public int OveralCollList23 { get; set; }
        public float OveralVList23 { get; set; }
        public int OveralCollList1 { get; set; }
        public float OveralVList1 { get; set; }
        public int OveralCollList2 { get; set; }
        public float OveralVList2 { get; set; }
        public int OveralCollList { get; set; }
        public float OveralVList { get; set; }
        public float PercentLossVeneer { get; set; }
        public float Rate1 { get; set; }
        public float Rate2 { get; set; }
        public float RateMedium { get; set; }
        public int ColBlock { get; set; }
        public int MediumDBlock { get; set; }
        public float VBlock { get; set; }
        public float VCil { get; set; }
        public float VCore { get; set; }
        public float LossCil { get; set; }
        public float LossCore { get; set; }
        public float MediumDCore { get; set; }
        public int Smena { get; set; }
    }
}
